...
def analyze_non_linear_trends(target_files_with_years):
    ...
    # Create a list to hold the plots we generate
    generated_plots = []

    # The expander and loop should NOT be here, but in the app.py.
    # We just perform the calculation and generate the figures.
    for from_cls in final_counts_template.index:
        for to_cls in final_counts_template.columns:
            ...
            # Generate the plot for significant trends
            if np.mean(rate_timeseries) > 100:
                ...
                # Add the generated figure to our list
    ...
    # Return the plots along with the other results
    return periods[-1]['rates'].reindex_like(final_counts_template), projected_total_counts.round().astype(int), generated_plots